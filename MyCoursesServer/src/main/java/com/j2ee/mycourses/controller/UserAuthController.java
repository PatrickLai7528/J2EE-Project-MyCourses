...
//        teacherEntity1.setTeacherNo(teacherEntity.getName());
        APIResponse apiResponse = new APIResponse();
        TeacherEntity test = new TeacherEntity();
        test.setTeacherEmail(teacherEntity.getTeacherEmail());
        test.setName(teacherEntity.getName());
        test.setPassword(teacherEntity.getPassword());
        test.setTeacherNo(teacherEntity.getTeacherNo());
        try {
            userAuthService.registry(test);
            apiResponse.setSuccess(true);
            apiResponse.setMessage("用戶" + test.getTeacherEmail() + "注冊成功");
