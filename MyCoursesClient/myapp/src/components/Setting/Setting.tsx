...
class UserProfileFrom extends React.Component<FormComponentProps, IUserProfileFromState> {
    ...
    public async componentDidMount() {
        const messageOfUserInfo: IAPIResponse<any> = await UserAPI.getInstance().getSimpleUserInfo();
        if (messageOfUserInfo.isSuccess)
            this.setState({userInfo: messageOfUserInfo.payload});

        const messageOfAreaCode: IAPIResponse<any> = await UserAPI.getInstance().getAreaCode();
        if (messageOfAreaCode.isSuccess) {
            const areaCodes = messageOfAreaCode.payload;
        ...
        const messageOfJobType: IAPIResponse<any> = await UserAPI.getInstance().getJobTypeOptions();
        if (messageOfJobType.isSuccess) {
            const jobTypeOptions = messageOfJobType.payload; // be care of this
